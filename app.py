...
# =========================================
# BSE fetch — day-by-day; NO category/subcategory filtering here
# =========================================
def fetch_bse_announcements_strict(start_yyyymmdd: str,
    ...
    NOTE: No deduplication and no category/subcategory filters here.
    """
        ...
        for v in variants:
            ...
            while True:
                ...
                if total and len(rows) >= total:
                    break
            if rows:
                # keep first variant with rows for that day (matches your original pattern)
                day_rows = rows
                break
        if day_rows:
...
with st.sidebar:
    ...
    run = st.button("🚀 Fetch (then choose filters below)", type="primary")
...
# Keep a place to store latest fetched DF so the dropdowns can be shown without re-fetching
...
if df_hits.empty:
    st.info("Pick your date range and click **Fetch**. Then you'll get Category/Subcategory dropdowns to filter before summarizing.")
else:
    # --- NEW: UI filters (user-controlled) ---
    st.subheader("🔎 Choose filters")
    cat_col = _first_col(df_hits, ["CATEGORYNAME","CATEGORY","NEWS_CAT","NEWSCATEGORY","NEWS_CATEGORY"])
    sub_col = _first_col(df_hits, ["SUBCATEGORYNAME","SUBCATEGORY","SUB_CATEGORY","NEWS_SUBCATEGORY","NEWS_SUB"])

    cats = sorted(df_hits[cat_col].dropna().astype(str).map(_norm).unique()) if cat_col else []
    subs = sorted(df_hits[sub_col].dropna().astype(str).map(_norm).unique()) if sub_col else []

    col1, col2 = st.columns(2)
    with col1:
        sel_cats = st.multiselect("Category (leave blank for all)", options=cats, default=[])
    with col2:
        sel_subs = st.multiselect("Subcategory (leave blank for all)", options=subs, default=[])

    df_filtered = df_hits.copy()
    if cat_col and sel_cats:
        df_filtered["_cat_norm"] = df_filtered[cat_col].astype(str).map(_norm)
        df_filtered = df_filtered[df_filtered["_cat_norm"].isin(set(sel_cats))].drop(columns=["_cat_norm"])
    if sub_col and sel_subs:
        df_filtered["_sub_norm"] = df_filtered[sub_col].astype(str).map(_norm)
    ...
    summarize_btn = st.button("🧠 Apply filters & Summarize", type="secondary")
    ...
    if summarize_btn:
        ...
        nm, subcol = _pick_company_cols(df_to_sum)
        ...
        def worker(idx, row, urls):
            ...
            summary = safe_summarize(pdf_bytes, company, headline, subcat,
                                     model=model,
                                     style=("bullets" if style=="bulgets" else "narrative"),
                                     max_output_tokens=int(max_tokens),
                                     temperature=float(temperature))
            return idx, used_url, summary, None

        # NOTE: small typo fix: "bulgets" -> "bullets"
        def worker(idx, row, urls):
            pdf_bytes, used_url = None, ""
            for u in urls:
                try:
            ...
            summary = safe_summarize(pdf_bytes, company, headline, subcat_val,
                                     model=model,
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as ex:
            ...
            for fut in as_completed(futs):
                ...
                with st.expander(f"{company or 'Unknown'} — {dt}  •  {subcat_val or 'N/A'}", expanded=False):
                    if headline:
